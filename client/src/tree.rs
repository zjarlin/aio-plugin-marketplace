...
pub(super) fn PluginTree(
    ...
    let entries: Vec<_> = entries
        ...
        .collect();
    let keys: Vec<_> = entries.iter().map(|entry| entry.git.clone()).collect();
    ...
    rsx! {
        ...
        div { role: "tree", aria_label: "插件", onkeydown: move |event: KeyboardEvent| {
            let current = keys.iter().position(|key| Some(key)==selection.as_ref()).unwrap_or(0);
            let next = match event.key() { Key::ArrowDown => Some((current+1).min(keys.len().saturating_sub(1))), Key::ArrowUp => Some(current.saturating_sub(1)), Key::Home => Some(0), Key::End => Some(keys.len().saturating_sub(1)), _ => None };
            ...
            for (installed,label) in [(true,"已安装"),(false,"可安装")] {
                details { class: "extension-browser__group", open: true,
                    summary { "{label} ({entries.iter().filter(|entry| entry.installed==installed).count()})" }
